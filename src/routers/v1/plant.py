...
async def get_plant_advice(code: str, user: User = Depends(current_user)):
    ...
    if plant.get("advice_updated_at"):
        advice_updated_at = datetime.fromisoformat(plant["advice_updated_at"])
        if advice_updated_at > datetime.now(timezone.utc) - timedelta(days=1):
            return {"advice": plant.get("advice", "")}
